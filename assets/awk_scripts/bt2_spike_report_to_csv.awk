...
BEGIN {
    cols="total_reads:bt2_spike_align1:bt2_spike_align_gt1:bt2_spike_non_aligned:bt2_spike_total_aligned"
    FS="\n"
    RS="\n\n"
...
}
{
   for (i=1; i<=NF; i++) {
       ...
       if(index($i,"aligned concordantly exactly 1 time") != 0) {
           split($i, line_split, " ");
           data["bt2_spike_align1"]=line_split[1];
       }
       if(index($i,"aligned concordantly >1 times") != 0) {
           split($i, line_split, " ");
           data["bt2_spike_align_gt1"]=line_split[1];
       }
       if(index($i,"aligned concordantly 0 times") != 0) {
           split($i, line_split, " ");
           data["bt2_spike_non_aligned"]=line_split[1];
       }
    }
}
END {
    data["bt2_spike_total_aligned"] = data["bt2_spike_align1"] + data["bt2_spike_align_gt1"]
